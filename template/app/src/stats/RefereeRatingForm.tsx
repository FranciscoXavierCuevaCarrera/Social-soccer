...
import { useAction, submitRefereeRating } from "wasp/client/operations";
...
export function RefereeRatingForm({
  refereeId,
  matchId,
  refereeName = "Árbitro asignado",
  matchTitle = "Partido",
}: RefereeRatingFormProps) {
  const submitRating = useAction(submitRefereeRating);
    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : "No se pudo registrar la evaluación arbitral.";
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-xl transition-all duration-300 dark:border-slate-700 dark:bg-[#2E3138]">
      ...
      {submitted ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {error && (
            <div className="rounded-xl border border-red-500/40 bg-red-50 p-3 text-xs font-medium text-red-700 dark:bg-red-950/30 dark:text-red-300">
              {error}
            </div>
          )}
